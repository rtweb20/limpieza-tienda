...
(function () {
  ...
  function demoApi(method, path, body) {
    ...
    if (method === 'POST' && path.endsWith('/ai/ask')) {
      return r({
        respuesta: 'Modo demostracion: tenes ' + demoDB.productos.length + ' productos cargados. Con el servidor activo y OPENAI_API_KEY configurada, este panel responde con IA usando el inventario real.',
        usandoIa: false,
        modelo: 'demo',
      });
    }
  ...
  function activarTab(tab) {
    ...
    if (tab === 'ia') cargarContextoIa();
  }

  async function cargarContextoIa() {
    if (!cacheProductos.length || !cacheCategorias.length || !cachePedidos.length) {
  ...
  async function preguntarIa(e) {
    ...
    const input = $('#aiPregunta');
    const btn = $('#aiAskBtn');
    const output = $('#aiRespuesta');
    ...
    output.innerHTML = '<p>Analizando inventario...</p>';

    try {
      const resp = await api('POST', '/api/admin/ai/ask', { pregunta });
      if (!resp) throw new Error('El asistente no devolvio respuesta.');
      badge.textContent = resp.usandoIa ? (resp.modelo || 'IA') : 'Analisis local';
      output.innerHTML = renderAiText(resp.respuesta);
    } catch (err) {
      output.innerHTML = `<p>${escapeHtml(err.message || 'No pude consultar el asistente.')}</p>`;
      toast('⚠️ ' + (err.message || 'Error en asistente IA'));
    } finally {
      btn.disabled = false;
      btn.innerHTML = '<span class="btn-icon">✨</span> Preguntar';
    }
  }
  ...
  function init() {
    ...
    $('#aiForm').addEventListener('submit', preguntarIa);
    $$('.ai-example, .ai-prompt').forEach((btn) => {
      btn.addEventListener('click', () => {
        $('#aiPregunta').value = btn.dataset.question || '';
        preguntarIa();
      });
    });
  }
